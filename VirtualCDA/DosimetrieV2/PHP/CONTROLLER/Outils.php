...
/**
 * Fonction particulière pour protection pour hierarchiser les données
 * Même argument que la précédente
 *
 * @param [type] $valeur
 * @param [type] $table
 * @param [type] $nomId
 ...
 * @return string
 */
function optionSelectGroupe($valeur, $table, $nomId, $colonne, $mode, $attribut, $filtres, $api)
{
    $groupement = ProtectionsManager::getList();;
    ...
    $select .= $attribut . '>';
    $liste = creerRequete($table, $nomId, "IdProtection," . $colonne, $filtres, $api);
    ...
    $protectionEnCours = ProtectionsManager::findbyId($liste[0]->getIdProtection());
    $select .= '<optgroup label =' . $protectionEnCours->getLibelleProtection() . '>';
    foreach ($liste as $elt) {
        if ($elt->getIdProtection() != $protectionEnCours->getIdProtection()) {
            $protectionEnCours = ProtectionsManager::findbyId($elt->getIdProtection());
            $select .= '</optgroup><optgroup label =' . $protectionEnCours->getLibelleProtection() . '>';
        }
...
//    $liste = creerRequete($table, $nomId, $colonne, $filtres, $api);
    $liste =DAO::select($colonnes,$table,null,null,null,$api,true);
	die(var_dump($liste));
    if ($valeur == null) { // si le code est null, on ne mets pas de choix par défaut avec valeur
        $select .= '<option value="" SELECTED>Choisir une valeur</option>';
    }
 ...
 * @return void
 */
function creerSelect(?int $valeur, string $table, array $nomColonnes, ?string $attributs = "", array $condition = null, string $orderBy = null,string $invite = "Choisir une valeur")
{
